...
uint8 K15_PosixSetThreadName(K15_Thread* p_Thread)
{
	...
	K15_ThreadContext* context = p_Thread->context;
	K15_PosixThread* posixThread = (K15_PosixThread*)p_Thread->userData;

	pthread_t threadHandle = posixThread->handle;

	uint8 result = K15_SUCCESS;

	if(pthread_setname_np(threadHandle, context->name) != 0)
	{
