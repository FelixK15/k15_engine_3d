...
intern uint32 K15_InternalPushGUIButtonVertices(K15_RenderBackEnd* p_RenderBackEnd, K15_GUIContext* p_GUIContext,
												K15_GUIElementHeader* p_GUIElement, float* p_VertexBuffer,
												uint32 p_VertexBufferIndexOffset)
{
	...
	float buttonStylePixelPosX	 = guiStyle->guiButtonStyle.posPixelX;
	float buttonStylePixelPosY	 = guiStyle->guiButtonStyle.posPixelY;
	float buttonStylePixelWidth	 = guiStyle->guiButtonStyle.pixelWidth;
	float buttonStylePixelHeight = guiStyle->guiButtonStyle.pixelHeight;
	...
	float elementMarginLeft	  = (float)guiStyle->guiButtonStyle.marginLeft;
	float elementMarginRight  = (float)guiStyle->guiButtonStyle.marginRight;
	float elementMarginTop	  = (float)guiStyle->guiButtonStyle.marginTop;
	float elementMarginBottom = (float)guiStyle->guiButtonStyle.marginBottom;
		...
		case K15_GUI_TYPE_BUTTON:
			{
				...
				vertexIndex = K15_InternalPushGUIButtonVertices(p_RenderBackEnd, p_GUIContext, 
									guiElement, p_VertexBuffer, vertexIndex);
