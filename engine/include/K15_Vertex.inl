...
/*********************************************************************************/
template<class Type>
Type Vertex::get(Enum p_Semantic)
{
	static uint32 typeSize = sizeof(Type);
	VertexElement element;
	Type returnVal;

	for(uint32 i = 0;i < m_Declaration->getElementCount();++i)
	{
		if(element.semantic == p_Semantic)
		{
			K15_ASSERT(element.size == typeSize,
			...
			uint32 bytesRead = m_Buffer->readData(typeSize,(byte*)returnVal,element.offset,true);
...
/*********************************************************************************/
template<class Type>
void K15_Engine::Rendering::Vertex::set(const Type& p_Value,Enum p_Semantic)
{
	static uint32 typeSize = sizeof(Type);
	VertexElement element;
	...
	for(uint32 i = 0;i < m_Declaration->getElementCount();++i)
	{
		if(element.semantic == p_Semantic)
		{
			...
			uint32 bytesWritten = m_Buffer->writeData(typeSize,(byte*)p_Value,element.offset);
